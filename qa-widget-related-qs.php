...
class qa_related_qs
{
	...
	public function output_widget($region, $place, $themeobject, $template, $request, $qa_content)
	{
		...
		require_once QA_PLUGIN_DIR.'qa-solr/qa-related-qs-find-overrides.php';
		...
		$count=qa_opt('page_size_related_qs');
		$count=isset($count) ? min($count, QA_DB_RETRIEVE_QS_AS) : QA_DB_RETRIEVE_QS_AS;
		$questions=qa_related_qs_find($userid, $questionid, $count, qa_match_to_min_score(qa_opt('match_related_qs')));
		$minscore=qa_match_to_min_score(qa_opt('match_related_qs'));

		foreach ($questions as $key => $question)
			if (isset($question['score']) && $question['score']<$minscore)
				unset($questions[$key]);

		qa_db_expand_questions($userid, $questions);
		...
		if ($region=='side') {
			...
			foreach ($questions as $question) {
				$acount = number_format($question['acount']);
				if (isset($question['selchildid']))
					$acount = "<span style='color:green'>$acount</span>";
				$themeobject->output('<li class="qa-related-q-item"><a href="'.qa_q_path_html($question['postid'], $question['title']).'">'.qa_html($question['title'])." <i>($acount)</i>".'</a></li>');
			}
