...
*/
	...
	//$dotenv = new Dotenv\Dotenv(__DIR__.'/../../', 'qa-config.sh');
	//$dotenv->load();
...
//shell_exec("ssh -i C:/Users/arun.manoharan/.ssh/id_rsa.infosys -p 2222 bitnami@infosys.nimeyo.com -L3307:mysql:3306 -L8983:solr:8983"); 
 


	define('QA_MYSQL_HOSTNAME', '127.0.0.1:3307'); // try '127.0.0.1' or 'localhost' if MySQL on same server
	define('QA_MYSQL_USERNAME', 'qpod_infosys');
	define('QA_MYSQL_PASSWORD', 'PpWiK77pQPJhXS8pSWdItWbwLqMi2rNj5n0M1bTP0VU=');
	define('QA_MYSQL_DATABASE', 'qpod_infosys');

	# For backward compatibility with existing backups, fix this password to qpod
	# but php 5.6+ requires this password length to be 16, 24 or 32
	# ref: http://stackoverflow.com/a/27254578/1533634
	define('QA_MCRYPT_PASSWORD', substr('qpod' . str_repeat("\0", 16), 0, 16));

/*
 *  ========================
 *    qPod's Configuration
 *  ========================
 */
